...
def check_stock(request):  # check còn game hay không
    try:
        ...
        return Response(
            {"out_of_stock": out_of_stock, "reserved": len(items) - len(out_of_stock)}
        )
    except Exception as e:
        return Response(
            {"error": f"An error occurred while checking stock - {e}."},
...
class CartView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):  # lấy context để dán vào data
        ...
        total_price = np.sum(game.price for game in games) + np.sum(
            dlc.price for dlc in dlcs
        )
        total_discounted_price = np.sum(
            game.discounted_price for game in games
        ) + np.sum(dlc.discounted_price for dlc in dlcs)

        context["orders"] = Order.objects.select_related("game", "dlc").filter(
            ...
            line_items.append(
                {
                    ...
                    if order.game
                    else order.dlc.stripe_price_id,
                    "quantity": 1,
                }
            )
...
def process_order(order, user, total_amount, session_id, customer_email, card_info):
    ...
    if order.game:
        ...
        send_mailjet_email_purchase_success(
            customer_email, order.game.name, order.id, order.key
        )
        ...
        send_mailjet_email_purchase_success(
            customer_email, order.dlc.name, order.id, order.key
        )
...
def webhook_view(request):
    ...
    event = verify_stripe_event(
        payload, sig_header, settings.STRIPE_WEBHOOK_SECRET, logger
    )
    if event is None:
    ...
    if len(line_items_data) != len(order_ids):
        logger.warning(
            f"[Webhook] Mismatch: {len(line_items_data)} items vs {len(order_ids)} orders."
        )
        return HttpResponse(status=400)
    ...
    for item, order_id in zip(line_items_data, order_ids):
        try:
            ...
            with transaction.atomic():
                if process_order(
                    order, user, total_amount, session_id, customer_email, card_info
                ):
                    processed_orders += 1
        except Order.DoesNotExist:
            logger.error(
                f"[Webhook] Order with ID {order_id} not found or not reserved."
            )
            continue
        except Exception as e:
